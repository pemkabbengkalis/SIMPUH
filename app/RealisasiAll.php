...
class RealisasiAll
{
    function cetak_realisasi_all($tahun = null, $trwangka = null)
    {
        $trw = isset([1 => 'I', 'II', 'III', 'IV'][$trwangka]) ? [1 => 'I', 'II', 'III', 'IV'][$trwangka] : '';
        $skpd = DB::table('tbl_skpd')->join('tbl_target', 'tbl_target.id_skpd', 'tbl_skpd.id_skpd')->join('tbl_realisasi', 'tbl_realisasi.id_skpd', 'tbl_skpd.id_skpd')->select('tbl_skpd.id_skpd', 'tbl_skpd.nama_skpd')->groupby('tbl_skpd.id_skpd')->orderBy('tbl_skpd.sort')->get();
        $realisasi = new RealisasiAll;
        // if(get_client_ip()=='103.231.115.123'){
        //     return $realisasi->get_detail_program(250, 2022, 2);
        //     }
        // foreach($skpd as $g){
        //     dd($realisasi->get_detail_program($g->id_skpd,2022));
        // }
        // return view('back.admin.report.pdf_realisasi_kabupaten',compact('realisasi','trw','skpd'));
        if(request()->exc){
            return view('back.admin.report.pdf_realisasi_kabupaten', compact('realisasi', 'trw', 'trwangka', 'skpd'));
        }
        $pdf = PDF::loadView('back.admin.report.pdf_realisasi_kabupaten', compact('realisasi', 'trw', 'trwangka', 'skpd'))->setOption('page-width', '215')->setOption('footer-right', 'Evaluasi Program Unggulan Kabupaten Bengkalis | Halaman [page] dari [topage]')->setOption('footer-font-size','7')->setOption('page-height', '330')->setOrientation('landscape');
        return $pdf->stream('REALISASI-TRIWULAN-' . $trw . '-KABUPATEN-BENGKALIS.pdf');
            ...
            foreach ($this->get_kegiatan($r->id_program, $id_skpd, $tahun) as $r2) {
                ...
                foreach ($this->get_sub_kegiatan($r2->id_kegiatan, $tahun) as $r3) {
                    ...
                    foreach ($arr_triwulan as $real) {
                        ...
                        $b['persen_kuantitas'] = Str::contains($qsbk['kuantitas'], ',') || $qsbk['kuantitas'] == 0
                        ? 0
                        : $b['persen_kuantitas'] + round(($this->get_realisasi_triwulan($qsbk['id_target'], $real, $qsbk['satuan'])['kuantitas'] / $qsbk['kuantitas']) * 100);


                        $b['persen_pagu'] = $qsbk['rp'] != 0 
                        ? $b['persen_pagu'] + round(($this->get_realisasi_triwulan($qsbk['id_target'], $real, $qsbk['satuan'])['pagu'] / $qsbk['rp']) * 100)
                        : $b['persen_pagu'];
